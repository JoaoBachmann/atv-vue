  ...
  <body>
    ...
    <div v-for="produto in produtos" :key="produto.id" class="livro">
      ...
      <p class="res">{{ produto.resenha }}</p>
        ...
        <br>
      <button class="compra" @click="booleano = !booleano">
        {{ valorBooleano ? `esconder resultado` : `Comprar` }}
            <i class="fa-solid fa-cart-shopping"></i>
      </button>      
    </div>
  </section>
  
  <section v-else class="conteudoOculto">
  <h1>Carrinho</h1>
  <p>Você ocultou os livros e informações.</p>
  <button class="mostrar" @click="booleano = true">
    Mostrar novamente
  </button>
</section>
...
* {
  font-family: 'SuaFonte', sans-serif; /* Substitua 'SuaFonte' pela fonte que você quer */
}
